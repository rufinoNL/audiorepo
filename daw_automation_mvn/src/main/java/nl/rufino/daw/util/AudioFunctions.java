...
public class AudioFunctions {
	...
	public static void analyzeAudioGenres(File folderToScan){
		String columnRegex = "[;]";
		String subColumnRegex = "[:]";
		String[] subColumnValues;
		LOGGER.info("Analyzing has started");
		LOGGER.info("Determining amount of folders");
		File[] existingFolders = folderToScan.listFiles((FileFilter) DirectoryFileFilter.DIRECTORY);
		
		LOGGER.info("For each folder search for Gereserveerd.txt"); //Gereserveerd.txt, needs to be renamed to data.txt
		for (File file : existingFolders) {
			File dataFile = new File(file.getAbsolutePath() + "\\Gereserveerd.txt");
			LOGGER.info("Reading: " + dataFile.getAbsolutePath());
			String dataFileRead = WindowsFunctions.openFile(dataFile);
			LOGGER.info("Found data: " + dataFileRead);
			String[] columnValues = dataFileRead.split(columnRegex);
			
			
			for (String columnValue : columnValues) {
				System.out.println("Kolomwaarde: " + columnValue);
				subColumnValues = columnValue.split(subColumnRegex);
			}
		}
